...
def get_or_create_area(value, block):
    if value:
        obj, created = Areas.objects.get_or_create(
            name=value,
            block=block
        )
        return obj
...
def get_or_create_areas_from_file(row):
    try:
        if pd.isnull(row["Block"]):
            ...
            get_or_create_area(value=row["Area_ID"].strip(), block=block)
    except Exception as e:
        print(e)

    # try:
    #     if "," in row["Area_ID"] or ";" in row["Area_ID"]:
    #         for area in row["Area_ID"].replace(";", ",").split(","):
    #             block = Blocks.objects.get(name=row["Block"])
    #             get_or_create_area(value=area.strip(), block=block)
    #
    #     else:
    #         block = Blocks.objects.get(name=row["Block"])
    #         get_or_create_area(value=row["Area_ID"].strip(), block=block)
...
def _create_blocks_from_file():
    ...
    df = pd.read_csv(file)
    df[~df["Area_ID"].isnull()].apply(lambda row: get_or_create_areas_from_file(row), axis=1)