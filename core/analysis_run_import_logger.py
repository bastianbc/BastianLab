...
class S3StorageLogHandler(logging.Handler):
    """Thread-safe handler that writes import logs to S3 or local Downloads folder as fallback."""
    ...
    def __init__(self, ar_name, sheet_name, total_files=None):
        ...
        self.lock = Lock()  # Thread-safe write lock
    ...
    def update_total_files(self, total_files):
        """Updates the total files count (thread-safe)."""
        with self.lock:
    ...
    def emit(self, record):
        """Write log record to buffer (thread-safe)."""
        try:
    ...
    def close(self):
        """Upload buffered logs to S3 (thread-safe)."""
        with self.lock:
            try:
